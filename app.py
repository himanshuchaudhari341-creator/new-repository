...
def scrape_site_content(url: str) -> dict:
    ...
    try:
        ...
        # Diagnose WHY content might be empty/thin, instead of showing a
        # vague "no content" message — this is either (a) a bot-blocking /
        # captcha interstitial, (b) a JS-only Single Page App with an empty
        # static HTML shell, or (c) a genuinely sparse page.
        low_text_signal = " ".join(raw_strings).lower()
        block_markers = [
        ...
        if content_points:
            ...
            clean_preview = "⚠️ This page's static HTML is nearly empty — it likely renders its real content via JavaScript (a Single Page App), which a standard scraper cannot see without a headless browser."
        else:
